...
Ox_ML210 <- read.xlsx("./Output/Table S5_ML210.xlsx", sheet = "OxPLs (filtered)") %>%
...
p_final <- ggplot(df, aes(x = Group, y = n, fill = FillCat)) +
  ...
  scale_fill_manual(
    ...
    labels = c(
      ...
      "Fenton"  = expression("Additional OxPLs identified using Fenton references"),
      "H2O2"    = expression("Additional OxPLs identified using" ~ H[2]*O[2] * "-only references"),
      "Overlap" = expression("Additional OxPLs identified in both Fenton and" ~ H[2]*O[2] * "-only references")
    )
  ) +
  scale_x_discrete(
    labels = c(
...
ggsave(
  ...
  plot   = p_final,
  width  = 1700/96, height = 420/96, units = "in",
