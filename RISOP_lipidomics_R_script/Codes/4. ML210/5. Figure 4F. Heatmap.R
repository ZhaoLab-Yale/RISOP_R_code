...
message("Oxidized lipids for heatmap: ", nrow(cmbd_lipids))
...
n_matched <- sum(rownames(df_heatmap) %in% ML210_clusters_fixed$Lipid)
message("Heatmap rows matched to cluster assignments: ", n_matched,
        " / ", nrow(df_heatmap))

row_cluster_df <- data.frame(Lipid = rownames(df_heatmap)) %>%
  left_join(ML210_clusters_fixed, by = "Lipid")

print(table(row_cluster_df$Cluster, useNA = "always"))

if (any(is.na(row_cluster_df$Cluster)))
  warning("Some heatmap rows have no cluster assignment: ",
          sum(is.na(row_cluster_df$Cluster)))

# 4.3 Sort rows by cluster then original order
row_cluster_df <- row_cluster_df %>%
  mutate(orig_order = row_number()) %>%
  arrange(factor(Cluster, levels = c("1", "2", "3")), orig_order)

df_heatmap_ordered <- df_heatmap[row_cluster_df$Lipid, ]
row_split          <- factor(row_cluster_df$Cluster, levels = c("1", "2", "3"))
...
cluster_levels <- levels(row_split)
row_titles     <- paste("Cluster", cluster_levels)
