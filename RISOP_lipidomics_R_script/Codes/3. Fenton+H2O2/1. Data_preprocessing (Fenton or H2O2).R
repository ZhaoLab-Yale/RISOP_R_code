...
reformat_plasmalogen <- function(lipid_vec) {
  case_when(
    str_detect(lipid_vec, "^LPC\\(O-\\d+:\\d+\\)$") ~ paste0(
      lipid_vec, " or LPC(P-",
      str_match(lipid_vec, "LPC\\(O-(\\d+):(\\d+)\\)")[,2], ":",
      as.integer(str_match(lipid_vec, "LPC\\(O-(\\d+):(\\d+)\\)")[,3]) - 1, ")"
    ),
    str_detect(lipid_vec, "^LPE\\(P-\\d+:\\d+\\)$") ~ paste0(
      "LPE(O-",
      str_match(lipid_vec, "LPE\\(P-(\\d+):(\\d+)\\)")[,2], ":",
      as.integer(str_match(lipid_vec, "LPE\\(P-(\\d+):(\\d+)\\)")[,3]) + 1,
      ") or ", lipid_vec
    ),
    TRUE ~ lipid_vec
  )
...
Ox_Fenton_prefiltered <- cmbd_lipids_for_supplementary %>%
  ...
  mutate(Removal = if_else(Alignment.ID %in% c("38015", "39402", "30767", "39995", "41440", "39887", "42220"),
                           "ISF", NA_character_)) %>%
...
list(
  `OxPLs (prefiltered)`        = Ox_Fenton_prefiltered,
  `NonOx_lipids (prefiltered)` = nonOx_Fenton_prefiltered,
  `OxPLs (filtered)`           = Ox_Fenton_filtered,
  `NonOx_lipids (filtered)`    = nonOx_Fenton_filtered
) %>%
  write.xlsx("./Output/Table S3_Fenton.xlsx")

list(
  `OxPLs (prefiltered)`        = Ox_H2O2_prefiltered,
  `NonOx_lipids (prefiltered)` = nonOx_H2O2_prefiltered,
  `OxPLs (filtered)`           = Ox_H2O2_filtered,
  `NonOx_lipids (filtered)`    = nonOx_H2O2_filtered
) %>%
  write.xlsx("./Output/Table S4_H2O2.xlsx")