...
# 1. Fenton ----
# =====================================================
PC_Fenton <- read.csv('./Input/20260221_Neg_FentonQC_PRM_1_PC_EIC.csv', stringsAsFactors = F, skip = 3, header = T)

start_time <- 8.2
end_time <- 10.5

df_plot <- data.frame(
  Time = as.numeric(PC_Fenton$Time.min.),
  Intensity = as.numeric(PC_Fenton$Intensity)
)
...
baseline_region <- df_plot[df_plot$Time >= 8.6 & df_plot$Time <= 8.8, ]
...
ggsave("./Figure/PC_Fenton_EIC.pdf", plot = a, width = 300/96, height = 250/96, device = "pdf")

# =====================================================
# 2. H2O2 ----
# =====================================================
PC_H2O2 <- read.csv('./Input/20260221_Neg_H2O2QC_PRM_1_PC_EIC.csv', stringsAsFactors = F, skip = 3, header = T)

start_time <- 8.2
end_time <- 10.5

df_plot <- data.frame(
  Time = as.numeric(PC_H2O2$Time.min.),
  Intensity = as.numeric(PC_H2O2$Intensity)
)
...
ggsave("./Figure/PC_H2O2_EIC.pdf", plot = b, width = 300/96, height = 250/96, device = "pdf")