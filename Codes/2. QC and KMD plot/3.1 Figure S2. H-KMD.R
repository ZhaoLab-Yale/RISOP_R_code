...
# ==============================================================================
# 8b. Measure the width a legend actually needs, in cm (GRID only) -----
# ==============================================================================
legend_width_cm <- function(panel_data,
  ...
  ox_names <- legend_df$Metabolite.curated[
    !legend_df$color_group %in% c("Non-oxidized", "ISF")]
  plain_names <- legend_df$Metabolite.curated[
    legend_df$color_group %in% c("Non-oxidized", "ISF")]
...
# ------------------------------------------------------------------------------
# 11a. GRID set -- multi-panel grid only -----
# ------------------------------------------------------------------------------
GRID_DOT_SIZE    <- 6
...
# ------------------------------------------------------------------------------
# 11b. SOLO set -- individual panels (sections 14-16) -----
# ------------------------------------------------------------------------------
SOLO_DOT_SIZE    <- 6
...
ISF_LINE_WIDTH_MIN <- 0.05
ISF_LINE_Y_OFFSET  <- 0.1
ISF_LINE_TYPE      <- "solid"
ISF_LINE_WIDTH     <- 0.3
...
xbreak_settings <- list(
  ...
  "PC(16:0_18:1)" = list(gap = c(11.3, 13), scale_ratio = 0.5)
)
...
build_panel <- function(grp, break_range = NULL, scale_ratio = "fixed") {
  ...
  x_breaks_panel <- seq(x_rng[1] + x_inset, x_rng[2] - x_inset,
                        length.out = GRID_N_XBREAKS)
  ...
  p_base <- ggplot(sub_df,
    ...
    geom_segment(data = subset(sub_df, !(color_group == "Non-oxidized" & is.na(precursor_label)) & color_group != "ISF"),
    ...
    theme(
      plot.title          = element_text(face = "bold", size = GRID_TITLE_SIZE, color = "black", hjust = 0.5,
  ...
  p <- p_base + labs(title = panel_title, x = "Retention time (min)", y = "H-KMD")
...
# ==============================================================================
# 13. Build and save the main grid (groups without axis break) -----
# ==============================================================================
...
# ==============================================================================
# 13b. Legend bottom alignment (for axis-break panels) -----
# ==============================================================================
...
p_nobreak <- ggplot(sub_df,
  ...
  labs(title = panel_title, x = "Retention time (min)", y = "H-KMD") +
  theme_minimal(base_size = 10) +
  theme(
...
# ==============================================================================
# 16. PC(16:0_9:0) as a standalone panel, no axis break-----
# ==============================================================================
grp        <- "PC(16:0_9:0)"
xlim_range <- xlim_settings[[grp]]
...
ggsave("./Figure/KMD_PC_16_0_18_1_break.pdf",
       plot   = final_PC_18_1,
       width  = 500*1.25/96,
       height = 280*1/96,
       units  = "in",
...
ggsave("./Figure/KMD_PC_16_0_16_1_break.pdf",
       plot   = final_PC_16_1,
       width  = 480*1.2/96,
       height = 280*1/96,
       units  = "in",
...
ggsave("./Figure/KMD_PC_16_0_9_0.pdf",
       plot   = final_PC_9_0,
