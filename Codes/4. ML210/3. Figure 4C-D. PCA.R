...
# ============================================================
# 2. Shared theme, colors, shapes, and time-point levels # -----
# ============================================================

# 2.1 Time-point levels, color gradient, and shapes
time_levels   <- c("0 min", "20 min", "45 min", "80 min", "120 min", "270 min")
teal_gradient <- c("#969696", "#FFCFFD", "#D296FF", "#9591FF", "#3A5FCD", "#262175")

# 2.2 Shape per time point (pch 21-25 are the only base R shapes with a separate
#     fill color; pch 10 [open star] takes an outline color only, no fill)
shape_values_time <- c(
  "0 min"   = 10,  # open star
  "20 min"  = 22,  # open square
  "45 min"  = 23,  # open diamond
  "80 min"  = 24,  # open triangle (up)
  "120 min" = 25,  # open triangle (down)
  "270 min" = 21   # open circle
)

# 2.3 Point stroke (line width) per time point -- only 0 min (star, pch 10)
#     needs a thicker outline since it has no fill
stroke_values_time <- c(
  "0 min"   = 1,
  "20 min"  = 0.5,
  "45 min"  = 0.5,
  "80 min"  = 0.5,
...
make_pca_plot_ml210 <- function(pca_result) {
  ...
  present_levels <- levels(droplevels(df_pcs$Group))
  stroke_present <- stroke_values_time[present_levels]
  
  ggplot(df_pcs, aes(x = PC1, y = PC2, color = Group, shape = Group,
                     fill = Group, stroke = Group)) +
    ...
    scale_discrete_manual(aesthetics = "stroke",
                          values     = stroke_values_time,
                          guide      = "none") +
    labs(x = pca_result$xaxistitle, y = pca_result$yaxistitle) +
    JCtheme_PCA() +
    guides(
      color = guide_legend(override.aes = list(size = 3.5, stroke = stroke_present)),
      shape = guide_legend(override.aes = list(size = 3.5, stroke = stroke_present)),
      fill  = guide_legend(override.aes = list(size = 3.5, stroke = stroke_present))
    )
}
