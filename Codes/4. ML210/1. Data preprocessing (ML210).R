...
cmbd_lipids <- rbind(impt_abs, impt_rela)
...
cmbd_lipids_pre <- rbind(impt_abs, impt_rela) %>%
  mutate(Ontology = case_when(
    str_detect(Metabolite.curated, "-") ~ {
      letter_before_dash <- str_extract(Metabolite.curated, "[A-Za-z](?=-)")
      paste0(Ontology, "-", letter_before_dash) %>%
...
# 11.5 Export
list(
  `OxPLs (prefilter)`        = Ox_ML210_prefiltered,
  `NonOx (prefilter)` = NonOx_ML210_prefiltered,
  `OxPLs (filtered)`           = Ox_ML210_filtered,
  `NonOx (filtered)`    = NonOx_ML210_filtered
) %>%
  write.xlsx("./Output/Table S5. Normalized intensities of OxPLs and molar concentrations of non-oxidized lipids identified in ML210-treated cells.xlsx")
