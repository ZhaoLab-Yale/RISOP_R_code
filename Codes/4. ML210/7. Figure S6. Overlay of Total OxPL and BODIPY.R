...
p_total_annotated <- ggplot(data_total_summary,
  ...
  geom_text(data = bracket_data,
            ...
            hjust = 0, vjust = 0.65, fontface = "bold",
            family = "Helvetica") +
  scale_size_identity() +
  ...
  guides(fill  = guide_legend(override.aes = list(size = c(3.5, 3.5)),
         ...
         color = guide_legend(override.aes = list(size = c(3.5, 3.5)),
                              keyheight = unit(1.3, "lines"))) +
  coord_cartesian(clip = "off", xlim = c(0.8, 7.2), ylim = c(-1.8, 2)) +
  theme_bw() +
  theme(
    text                 = element_text(family = "Helvetica"),
    axis.text            = element_text(size = 11.5),
    axis.text.x          = element_text(angle = 0, hjust = 0.5),
    axis.title           = element_text(size = 13.05, face = "bold"),
...
pdf("./Figure/overlay_of_total_OxPLs_and_Bodipy.pdf", width = 645*1.0 / 96, height = 388*1.0 / 96)
print(p_total_annotated)
dev.off()
