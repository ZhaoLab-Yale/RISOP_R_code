...
# ============================================================
# 2. Shared theme, colors, shapes, sizes, and labels # -----
# ============================================================
...
# 2.2 Combined Group_Time levels (Fenton first, then H2O2)
combined_levels <- c(
  "Fenton_0 h",  "Fenton_1 min",  "Fenton_20 min",  "Fenton_4 h",  "Fenton_24 h",  "Fenton_72 h",
  "H2O2_0 h",    "H2O2_1 min",    "H2O2_20 min",    "H2O2_4 h",    "H2O2_24 h",    "H2O2_72 h"
)

# 2.3 Fill color: 2 colors only, repeated across the 6 time points of each group
fill_values_combined <- setNames(
  c(rep("#E05A51", 6), rep("#377EB8", 6)),
  combined_levels
)

# 2.4 Shape per time point (open shapes; pch 21-25 are the only base R shapes
#     with a separate fill color, so the 6th time point uses an open star
#     [pch 8] which only takes an outline color, no separate fill)
shape_values_time <- c(
  "0 h"    = 10,  # open star
  "1 min"  = 22,  # open square
  "20 min" = 23,  # open diamond
  "4 h"    = 24,  # open triangle (up)
  "24 h"   = 25,  # open triangle (down)
  "72 h"   = 21    # open circle
)
shape_values_combined <- setNames(rep(shape_values_time, 2), combined_levels)

# 2.5 Point size per Group_Time -- edit these values to resize individual points
point_size_values <- c(
  "Fenton_0 h"    = 3,
  "Fenton_1 min"  = 4,
  "Fenton_20 min" = 4,
...
# 2.5b Point stroke (line width) per Group_Time -- only 0 h (star, pch 8) needs thicker outline
stroke_values_combined <- c(
  "Fenton_0 h"    = 1,
  "Fenton_1 min"  = 0.5,
  "Fenton_20 min" = 0.5,
  "Fenton_4 h"    = 0.5,
...
# 2.6 Legend labels, using expression() with subscript for the H2O2 chemical formula
...
# 3.1 Run PCA for a single dataset (Fenton-only or H2O2-only)
run_pca_single <- function(cmbd_lipids, Sample_info, group_name) {
  df <- cmbd_lipids %>%
    ungroup() %>%
    select(-Ontology) %>%
  ...
  df_pcs <- df_pcs %>%
    mutate(Group_Time = factor(paste(group_name, Time, sep = "_"), levels = combined_levels))
  
  list(df_pcs   = df_pcs,
       variance = variance)
}

# 3.2 Run PCA for combined dataset (Fenton + H2O2)
run_pca_combined <- function(df_features, Sample_info) {
  ...
  df_pcs <- df_pcs %>%
    mutate(Group_Time = factor(paste(Group, Time, sep = "_"), levels = combined_levels))
  
  list(df_pcs   = df_pcs,
       variance = variance)
}

# 3.3 Build PCA plot (shared by single-group and combined datasets)
make_pca_plot <- function(pca_result) {
  ...
  present_levels <- levels(droplevels(df_pcs$Group_Time))
  stroke_present <- stroke_values_combined[present_levels]
  
  ggplot(df_pcs, aes(x = PC1, y = PC2,
                     shape = Group_Time, fill = Group_Time, color = Group_Time,
                     size = Group_Time, stroke = Group_Time)) +
    ...
    scale_color_manual(name   = "Conditions",
                       values = fill_values_combined,
                       labels = custom_labels_combined) +
    scale_size_manual(values = point_size_values, guide = "none") +
    scale_discrete_manual(aesthetics = "stroke", values = stroke_values_combined, guide = "none") +
    JCtheme_PCA() +
    guides(
      shape = guide_legend(override.aes = list(size = 3.5, stroke = stroke_present, alpha = 0.7)),
      fill  = guide_legend(override.aes = list(size = 3.5, stroke = stroke_present, alpha = 0.7)),
      color = guide_legend(override.aes = list(size = 3.5, stroke = stroke_present, alpha = 0.7))
    )
}
...
pca_H2O2_Ox    <- run_pca_single(cmbd_lipids_H2O2 %>% filter( grepl("<", Metabolite.curated, fixed = TRUE)), Sample_info, group_name = "H2O2")
pca_H2O2_nonOx <- run_pca_single(cmbd_lipids_H2O2 %>% filter(!grepl("<", Metabolite.curated, fixed = TRUE)), Sample_info, group_name = "H2O2")

pca_Fenton_Ox    <- run_pca_single(cmbd_lipids_Fenton %>% filter( grepl("<", Metabolite.curated, fixed = TRUE)), Sample_info, group_name = "Fenton")
pca_Fenton_nonOx <- run_pca_single(cmbd_lipids_Fenton %>% filter(!grepl("<", Metabolite.curated, fixed = TRUE)), Sample_info, group_name = "Fenton")

# ============================================================
# 7. Build all 6 PCA plots # -----
# ============================================================

plot_combined_Ox    <- make_pca_plot(pca_combined_Ox)
plot_combined_nonOx <- make_pca_plot(pca_combined_nonOx)

plot_H2O2_Ox    <- make_pca_plot(pca_H2O2_Ox)
plot_H2O2_nonOx <- make_pca_plot(pca_H2O2_nonOx)

plot_Fenton_Ox    <- make_pca_plot(pca_Fenton_Ox)
plot_Fenton_nonOx <- make_pca_plot(pca_Fenton_nonOx)
